...
The release review also found that the production newsletter and contact providers had not been configured. Shared signup modules now show a clear paused state with newsletter sample routes when the selected provider lacks credentials. The contact page offers the existing hello@arahkaii.com email route, including a partnership subject, until its sender is configured. No subscriber data is collected by an unavailable form.

To activate the existing integrations, add `MAILERLITE_API_KEY` (and optionally `MAILERLITE_GROUP_ID`) for the selected MailerLite account, and `RESEND_API_KEY` with the appropriate `CONTACT_FROM_EMAIL` / `CONTACT_TO_EMAIL` for contact delivery, to the production Vercel environment. Redeploy and verify provider delivery using an authorised test recipient before treating those conversions as operational. Credentials must remain outside Git. Existing advertising and consent configuration is retained.
