...
for (const file of htmlFiles) {
	...
	if (!/<[^>]+\bclass="[^"]*\bsearchbar\b[^"]*"[^>]+\binert(?:\s|>|=)/.test(html)) {
		collapsedSearchFailures.push(`${relativeFile}: collapsed site search is not inert`);
	}
...
if (collapsedSearchFailures.length) {
	console.error('\nCollapsed search failures:');
	for (const item of collapsedSearchFailures) console.error(`- ${item}`);
}
...
if (
	...
	collapsedSearchFailures.length ||
